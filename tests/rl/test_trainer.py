...
from lerobot.rl.algorithms.base import RLAlgorithm, SamplingSpec, TrainingStats
...
class _RecordingMixer:
    ...
    def get_iterator(self, batch_size: int, async_prefetch: bool = True, queue_size: int = 2, **kwargs):
...
class _SamplingAlgorithm(_CountingAlgorithm):
    def get_sampling_spec(self) -> SamplingSpec | None:
        return SamplingSpec(n_steps=5, gamma=0.97)
...
def test_algorithm_sampling_spec_is_forwarded_to_iterator():
    algo = _SamplingAlgorithm()
    mixer = _RecordingMixer()
    trainer = RLTrainer(algorithm=algo, data_mixer=mixer, batch_size=3, async_prefetch=False, queue_size=4)

    trainer.training_step()

    assert mixer.iterator_kwargs is not None
    assert mixer.iterator_kwargs["batch_size"] == 3
    assert mixer.iterator_kwargs["async_prefetch"] is False
    assert mixer.iterator_kwargs["queue_size"] == 4
    assert mixer.iterator_kwargs["n_steps"] == 5
    assert mixer.iterator_kwargs["gamma"] == 0.97
